    ...
    size_t index, const void *item) 
{
    ...
    printf("slot=0x%lx, item=0x%lx\n", *(uint64_t *)slot, (uint64_t) item);
}
    ...
    struct btree_node *node, const void *key, bool *found) 
{
    ...
    while ( i < n ) {
        size_t j = (i + n) >> 1;
        void *item = btree_get_item_at((struct btree *) btree, node, j);
    ...
    int depth) 
{
    ...
    if (hint && depth < 8) {
        ...
        if (index > 0) {
            ...
            void *item = btree_get_item_at((struct btree *) btree, node, (size_t)index);
            int cmp = _btree_compare(btree, key, item);
            if (cmp == 0) {
                *found = true;
                return index;
    ...
    while ( low <= high ) {
        int mid = (low + high) / 2;
        void *item = btree_get_item_at((struct btree *) btree, node, (size_t)mid);
        int cmp = _btree_compare(btree, key, item);
        if (cmp == 0) {
            *found = true;
            index = mid;
    ...
    void *udata)
{
    ...
    struct btree *btree = (struct btree *) malloc_(size);
    if (!btree) {
        return NULL;
    }
    memset(btree, 0, size);
...
static struct btree_node *btree_node_new(struct btree *btree, bool leaf) {
    ...
    struct btree_node *node = (struct btree_node *) btree->malloc(size);
    if (!node) {
        return NULL;
    }
    memset(node, 0, size);
...
struct btree *btree_clone(struct btree *btree) {
    ...
    struct btree *btree2 = (struct btree *) btree->malloc(size);
    if (!btree2) {
        return NULL;
    }
...
set:
    ...
    old_root = btree->root;
    new_root = btree_node_new(btree, false);
    if (!new_root) {
        goto oom;
    }
    ...
    uint64_t *hint)
{
    ...
    while (1) {
        ...
        if (found) {
            return btree_get_item_at((struct btree *)btree, node, i);
        }
        if (node->leaf) {
            return NULL;
        }
    ...
    size_t index, const void *key, uint64_t *hint) 
{
    ...
    btree_cow_node_or(btree->root, goto oom);
    result = btree_node_delete(btree, btree->root, act, 
    ...
    bool (*iter)(const void *item, void *udata), void *udata)
{
    if (node->leaf) {
        for (size_t i = 0; i < node->nitems; i++) {
            if (!iter(btree_get_item_at((struct btree *) btree, node, i), udata)) {
                return false;
            }
        }
        return true;
...
const void *btree_min(const struct btree *btree) {
    ...
    while (1) {
        if (node->leaf) {
            return btree_get_item_at((struct btree *) btree, node, 0);
        }
        node = node->children[0];
    }
}
...
const void *btree_max(const struct btree *btree) {
    ...
    while (1) {
        if (node->leaf) {
            return btree_get_item_at((struct btree *) btree, node, node->nitems-1);
        }
        node = node->children[node->nitems];
    }
}
...
struct btree_iter *btree_iter_new(const struct btree *btree) {
    ...
    struct btree_iter *iter = (struct btree_iter *) btree->malloc(vsize + btree->elsize);
    if (iter) {
...
bool btree_iter_last(struct btree_iter *iter) {
    ...
    struct btree_node *node = iter->btree->root;
    while (1) {
        struct btree_iter_stack_item item;
        item.node = node;
        item.index = node->nitems;
